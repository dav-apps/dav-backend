class ValidationService
	...
	def self.validate_content_type_json(content_type)
		return get_validation_hash(false, 1101, 400) if content_type.nil?
		return get_validation_hash(false, 1003, 415) if !content_type.include?("application/json")
		get_validation_hash
	end

	def self.validate_content_type_image(content_type)
		return get_validation_hash(false, 1101, 400) if content_type.nil?
		return get_validation_hash(false, 1003, 415) if !content_type.include?("image/png") && !content_type.include?("image/jpeg")
		get_validation_hash
	end

	def self.validate_content_type_supported(content_type)
		return get_validation_hash(false, 1101, 400) if content_type.nil?
		return get_validation_hash(false, 1003, 415) if content_type.include?("application/x-www-form-urlencoded")
		get_validation_hash
	end
	...
	def self.raise_image_file_invalid
		raise RuntimeError, [get_validation_hash(false, 1201, 400)].to_json
	end
	...
	def self.validate_first_name_length(first_name)
		if first_name.length < Constants::FIRST_NAME_MIN_LENGTH
			get_validation_hash(false, 2300, 400)
		elsif first_name.length > Constants::FIRST_NAME_MAX_LENGTH
			get_validation_hash(false, 2400, 400)
		else
			get_validation_hash
		end
	end
	...
	def self.get_session_from_token(token, check_renew = true)
		...
		if session.nil?
			...
			if session.nil?
				...
				raise RuntimeError, [get_validation_hash(false, 3603, 404)].to_json
			else
				# The old token was used
				# Delete the session, as the token may be stolen
				session.destroy!
				raise RuntimeError, [get_validation_hash(false, 3100, 403)].to_json
			end
		elsif check_renew
			# Check if the session needs to be renewed
			if (Time.now - session.updated_at) > 1.day
				raise RuntimeError, [get_validation_hash(false, 3101, 403)].to_json
			end
		end
